...
def logout(requset: Request, response: Response, csrf_protect: CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(requset.headers)
    csrf_protect.validate_csrf(csrf_token)
    response.set_cookie(
...
def get_user_refresh_jwt(requset: Request, response: Response):
    new_token, subject = auth.verify_update_jwt(requset)
    response.set_cookie(
