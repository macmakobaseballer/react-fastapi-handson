...
from database import db_create_todo, db_get_todos, db_get_todo, db_update_todo, db_delete_todo
...
async def create_todo(request: Request, response: Response, data: TodoBody, csrf_protect: CsrfProtect = Depends()):
    ...
    if res:
        return res
    raise HTTPException(
        status_code=400, detail="Create task failed"
    )
...
async def get_todo(request: Request, response: Response, id: str):
    ...
    res = await db_get_todo(id)
    response.set_cookie(
        key="access_token", value=f"Bearer {new_token}", httponly=True, samesite="none", secure=True)
    if res:
        return res
    raise HTTPException(
        status_code=404, detail=f"Task of {id} doesn't exist"
    )
